...
import {changeOpenChannelStatus, makeStartup} from "./actions/global";
...
import SmartWalletModalComponent from "./components/SmartWalletModalComponent";
import {switchEnvelopingStatus, switchSmartWalletModal} from "./actions/enveloping";
import DeploySmartWalletModalComponent from "./components/DeploySmartWalletModalComponent";
import RelayTransactionModalComponent from "./components/RelayTransactionModalComponent";
...
class App extends React.Component {
    ...
    switchEnveloping = async () => {
        this.props.switchEnvelopingStatus();
    }

    switchEnvelopingModal = async () => {
        this.props.switchSmartWalletModal();
    }
    ...
    getButtons() {
        ...
        const enableEnveloping = (
            <li key="switch-enveloping" className="mx-2">
                <Button onClick={this.switchEnveloping}>
                    {this.props.usingEnveloping ? "Enveloping ON" : "Enveloping OFF"}
                    <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="power-off"
                         className="svg-inline--fa fa-power-off fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg"
                         viewBox="0 0 640 512"
                         width={20}>
                        <path fill={this.props.usingEnveloping ? "#32b438" : "#b43232"}
        ...
        const smartWalletsModal = (
            <li key="switch-smart-wallets" className="mx-2">
                <Button onClick={this.switchEnvelopingModal}>
                    Smart Wallets
                    <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="wallet"
                         className="svg-inline--fa fa-wallet fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg"
                         viewBox="0 0 640 512"
                         width={20}>
        ...
        return showButtons ? [
            enableEnveloping,
            this.props.usingEnveloping ? smartWalletsModal : null,
            (
    ...
    getInfo() {
        const enveloping = Lumino.getConfig().enveloping;
        let notifierItem = null;
                ...
                <li key="notifier-endpoints">
                    ...
                    {notifierEndpoints.map((notifierEndpoint, index) => {
                        if (index < notifierEndpoints.length - 1) {
                            return (<span key={`notifier-endpoint-${index}`}><a key={`notifier-endpoint-link-${index}`}
                                                                                href={notifierEndpoint}>{notifierEndpoint}</a>,&nbsp;</span>);
                        }
                        return (<span key={`notifier-endpoint-${index}`}><a key={`notifier-endpoint-link-${index}`}
                                                                            href={notifierEndpoint}>{notifierEndpoint}</a></span>);
                    })}
        ...
        let envelopingItems = null;
        if (enveloping) {
            envelopingItems = [<li key="empty-line"><br/></li>, <li key="enveloping">ENVELOPING</li>];
            const preferredRelaysItem = (
                <li key="preferred-relays">
                    Preferred Relay Endpoints:&nbsp;
                    {preferredRelays.map((relayEndpoint, index) => {
                        if (index < preferredRelays.length - 1) {
                            return (<span key={`relay-endpoint-${index}`}><a key={`relay-endpoint-link-${index}`}
                                                                             href={relayEndpoint}>{relayEndpoint}</a>,&nbsp;</span>);
                        }
                        return (<span key={`relay-endpoint-${index}`}><a key={`relay-endpoint-link-${index}`}
                                                                         href={relayEndpoint}>{relayEndpoint}</a></span>);
                    })}
            ...
            envelopingItems.push(<li key="relay-verifier">Relay Verifier Contract
                Address: {relayVerifierContractAddress}</li>);
            envelopingItems.push(<li key="deploy-verifier">Deploy Verifier Contract
                Address: {deployVerifierContractAddress}</li>);
            envelopingItems.push(<li key="relay-hub">Relay Hub Contract Address: {relayHubContractAddress}</li>);
            envelopingItems.push(<li key="smart-wallet">Smart Wallet Factory Contract
                Address: {smartWalletFactoryContractAddress}</li>);
            envelopingItems.push(preferredRelaysItem);
        }
        return (
            <div className="network-info">
                <ul className="list-unstyled">
                    <li key="lumino">LUMINO</li>
                    <li key="address">Address: {address}
                        <svg onClick={this.copyAddressToClipboard} aria-hidden="true" focusable="false"
    ...
    getEnvelopingModals() {
        if (this.props.usingEnveloping) {
            const modals = [];
            if (this.props.isDeployingSmartWallet) {
                modals.push(<DeploySmartWalletModalComponent/>);
            } else if (this.props.isRelayingTransaction) {
                modals.push(<RelayTransactionModalComponent/>)
            } else {
                modals.push(<SmartWalletModalComponent/>);
            }
            return modals;
        }
        return null;
    ...
    render = () => {
        ...
        const envelopingModals = this.getEnvelopingModals();
        return (
...
const mapStateToProps = state => {
    return {
        ...
        isDeployingSmartWallet: state.enveloping.openedDeploySmartWalletModal && state.enveloping.selectedForDeploy,
        isRelayingTransaction: state.enveloping.openedRelayTransactionModal
    };
};
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        switchEnvelopingStatus: () => dispatch(switchEnvelopingStatus()),
        switchSmartWalletModal: () => dispatch(switchSmartWalletModal())
    };
}
